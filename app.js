(function(){
  ...
  function renderHistory() {
    if (!historyEl) return;
    if (history.length === 0) {
      historyEl.innerHTML = '<li class="muted">Aucun changement récent</li>';
      return;
    }
    historyEl.innerHTML = history.map(item => `<li>• Ajustement : <strong>${item}</strong></li>`).join('');
  }
  ...
  btnCamp.addEventListener('click', ()=>{
    ...
    const delta = (state.world === 'reflet') ? -20 : -30;
    state.campLeft -= 1;
    state.value = clamp(state.value + delta);
